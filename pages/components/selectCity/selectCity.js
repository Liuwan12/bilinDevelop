...
const app = getApp();
const SEARCH_CITY ='/plate/getCityInfoByName';
const GET_HOT_CITY ='/plate/getAllCity';
...
Component({
  ...
  methods: {
    ...
    inputCity(e){
      var that = this;
      wxRequest.getRequest(app.globalData.Api + SEARCH_CITY, { name: e.detail.value}).then(res => {
        that.setData({
          inputcity: res.data.data ? res.data.data:[]
          });
      });
    },
    //取消搜索
    ...
    getHotCity() {
      var that = this;
      wxRequest.getRequest(app.globalData.Api + GET_HOT_CITY).then(res=>{
        that.setData({
          newcity: res.data.data.firecity
        });
      })
    },
    //点击城市
    ...
    CityList(){
      var _this = this;
      new qqmap().getCityList().then(function (val) {//这个方法在另一个文件里
        console.log(val.result);
        // var citylists = val.result[1];
        var citylists = [{'cityName':'北京市','pinyin':['bei','jing']},
                        {'cityName':'上海市','pinyin':['shang','hai']},
                        {'cityName':'天津市','pinyin':['tian','jin']},
                        {'cityName':'重庆市','pinyin':['chong','qing']},
                        { 'cityName':'香港岛','pinyin':['xiang','gang']},
                        {'cityName':'澳门半岛','pinyin':['ao','men']}];
        for (var i = 0; i < val.result[1].length; i++) {
          ...
          // }
          if (spliceCity != '110' && spliceCity != '120' && spliceCity != '310' && spliceCity != '500' && spliceCity != '810' && spliceCity != '820' ) {
            citylists.push({'cityName':val.result[1][i].fullname,'pinyin':val.result[1][i].pinyin});
          }
        }
